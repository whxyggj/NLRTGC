function [x,objV] = wshrinkObj_tanh(x,rho,sX, isWeight,mode,alfa,theta)
...
if isinteger(n3/2)
    ...
    for i = 1:endValue
        ...
        if isWeight
            ...
            shat = soft_threshold(shat,diag(tau));
        else
            tau = rho;
            shat = max(shat - tau,0);
        end                 
   ...
   endValue = int16(n3/2+1);
    for i = 1:endValue
        ...
        if isWeight
            ...
            sigma = diag(shat);
            w = alfa*(1-theta*tanh(theta*sigma).^2);
            tau = rho.*w;
            shat = max(shat - diag(tau),0);
        end
        objV = objV + sum(shat(:));
        Yhat(:,:,i) = uhat*shat*vhat';
        if i > 1
