clear
clc
clearvars -except ACCs NMIs ARs Fs Ps Rs tun AG 
...
while iter < max_iter
    ...
    %% update E
    EF = [T{1}-Z{1}+FI{1}/mu;T{2}-Z{2}+FI{2}/mu;T{3}-Z{3}+FI{3}/mu];
...
% toc
for i=1:10
    C{i} = SpectralClustering(P_hat,numClust);
...
plt = 1:1:iter+1;
figure
plot(plt,leqmv,'r*-','LineSmoothing','on');
hold on;
plot(plt,difEv,'b*-','LineSmoothing','on');
hold on;
plot(plt,difZv,'g*-','LineSmoothing','on')
hold on;
legend('||P-Z_{k+1}-E_{k+1}||_{\infty}','||E_{k+1}-E_{k}||_{\infty}','||Z_{k+1}-Z_{k}||_{\infty}');
set(gca,'FontName','Times New Roman','FontSize',14,'FontWeight','normal');
xlabel('Iteration', 'FontName','Times New Roman','FontSize',14,'FontWeight','normal')  %xÖá×ø±êÃèÊö
ylabel('Error', 'FontName','Times New Roman','FontSize',14,'FontWeight','normal') %yÖá×ø±êÃèÊö
